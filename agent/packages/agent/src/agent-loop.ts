				...
				if (upstreamRetries < UPSTREAM_RETRY_LIMIT) {
					...
					pendingMessages.push({
						...
						content: [
							{
								type: "text",
								text: "Transient upstream failure occurred. Resume by calling a tool directly — avoid prose. Prioritize correctness, and report blockers clearly if no safe edit is possible.",
							},
						],
						timestamp: Date.now(),
					});
						...
						content: [
							{
								type: "text",
								text: tokenCapped
									? "Output budget consumed without any tool invocation. Invoke \`read\`, \`edit\`, or \`write\` now, or briefly explain why no tool action is appropriate."
									: "No file modifications detected yet. If you have a safe, high-confidence change, apply it now; otherwise, report the blocker or uncertainty explicitly.",
							},
						],
						timestamp: Date.now(),
					});
				...
				if (!hasProducedEdit && explorationCount >= dynamicExploreCeiling && pendingMessages.length === 0) {
					pendingMessages.push({
						...
						content: [
							{
								type: "text",
								text: `Context gathered (${explorationCount} reads/bashes). If evidence is sufficient, apply your first high-confidence file change (\`edit\` or \`write\`) on the highest-priority target.`,
							},
						],
						timestamp: Date.now(),
					});
				...
				if (
					...
					pendingMessages.push({
						...
						content: [
							{
								type: "text",
								text: `Discovery stall: ${totalExplorationSteps} read/bash steps with no \`edit\`/\`write\` yet. The top-ranked target is \`${primary}\` — \`read\` it if needed, then either make a justified change or explicitly state why no safe change should be made.`,
							},
						],
						timestamp: Date.now(),
					});
					...
					if (!earlyNudgeSent && elapsed >= EARLY_NUDGE_MS) {
						...
						pendingMessages.push({
							...
							content: [
								{
									type: "text",
									text: `${Math.round(elapsed / 1000)}s elapsed without any successful file changes. ${readList}Apply \`edit\` or \`write\` only if confidence is adequate; otherwise report the blocker or uncertainty and stop safely.`,
								},
							],
							timestamp: Date.now(),
						});
						...
						pendingMessages.push({
							...
							content: [
								{
									type: "text",
									text: `${Math.round(elapsed / 1000)}s in with zero file modifications. Time may be running out. ${readList}Prioritize a correct result over forced edits; if no safe change is supported, explicitly report why.`,
								},
							],
							timestamp: Date.now(),
						});
				...
				if (
					...
					pendingMessages.push({
						...
						content: [
							{
								type: "text",
								text: "Over 50s without successful file changes. Pick the clearest path from the task or keyword list and proceed only with a justified, high-confidence edit; otherwise provide a concise blocker report.",
							},
						],
						timestamp: Date.now(),
					});
