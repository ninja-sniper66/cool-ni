...
- If the task asks for implementation but confidence is insufficient after reasonable exploration, do **not** force speculative edits; report blockers and uncertainty clearly.
...
- **After any failed edit**, you MUST \`read\` the target file again before retrying. Never repeat the same \`oldText\` from memory or an outdated read; that produces repeated tool errors and stalls progress.
...
Before stopping:
- if no safe high-confidence edit is supported by evidence, stop and report the blocker instead of forcing a change
...
- either apply the highest-confidence minimal valid edit **or** report why no safe edit is justified yet
- prefer in-place changes near existing sibling logic
- avoid additional exploration loops
- when uncertain, correctness and safety outrank forcing a diff
- if \`edit\` or \`write\` fails, **read** and retry with corrected anchors; stop and report if safe progress is not possible
...
Correctness and safety outrank producing a non-empty diff. Avoid speculative edits when evidence is insufficient.
